...
AOIS = { 
    "Area1": (0, 0, 1920, 302),
    "Area2": (0, 302, 1920, 957),
    "Area3": (0, 957, 1920, 1080)
}
...
with open(csv_file, encoding="latin1") as f:
    ...
    for line in f:
        ...
        if first_column.startswith("fin_pagina"):
            fin_pagina_count += 1
            print(f"Detectado fin_pagina número {fin_pagina_count} en la línea {line_number}.")
            
            if fin_pagina_count == 2:
                # comenzamos a guardar a partir de la segunda aparición
                recording = True
                print(f"Comenzamos a grabar desde la línea {line_number+1}")
                continue  # no guardar la propia línea de fin_pagina
            elif fin_pagina_count == 3:
                # paramos en la tercera aparición
                print(f"Paramos de grabar en la línea {line_number}.")
                break
...
print(f"Se han leído desde la línea marcada tras la segunda fin_pagina hasta justo antes de la tercera fin_pagina, total {len(df)} filas.")
...
img_file = "/Users/carolinamachonieto/Desktop/TFG/PaginaWeb2.png"
